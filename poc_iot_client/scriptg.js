var runSocketIOSample = function() {
  ...
  L.Logger.info('position: ', position, ',vin: ', vin);
      ...
      if (shareButton) {
        ...
        shareButton.onclick = (function() {
          Woogeen.LocalStream.create({
            ...
            stream.attr("position", position);

            remoteVideo.src = window.URL.createObjectURL(stream.stream);

            stream.attr("vin", vin);
            L.Logger.info('set position 0:', position);
        ...
        if (typeof mediaUrl === 'string' && mediaUrl !== '') {
          ...
          if (isPublish !== 'false') {
            Woogeen.LocalStream.create({
              ...
              localStream.attr("position",position);
              localStream.attr("vin",vin);

              L.Logger.info('set position1:', position);              
